...
const getOpenSquares = function (info) {
  ...
  if (JSON.stringify(north) === JSON.stringify(neck) || north.y < 0 || filledSquares.includes(JSON.stringify(north))) {
    openSquares[0] = false;
  } 
  if (JSON.stringify(east) === JSON.stringify(neck) || east.x > info.board.width - 1 || filledSquares.includes(JSON.stringify(east))) {
    openSquares[1] = false;
  } 
  if (JSON.stringify(south) === JSON.stringify(neck) || south.y > info.board.length - 1 || filledSquares.includes(JSON.stringify(south))) {
    openSquares[2] = false;
  } 
  if (JSON.stringify(west) === JSON.stringify(neck) || west.x < 0 || filledSquares.includes(JSON.stringify(north))) {
    openSquares[3] = false;
  }  
